...
function SaleOrderDetails(): JSX.Element {
  ...
  const handleApproveOrder = useCallback(async () => {
    try {
      await approveOrder({ order_id: order.id });
  ...
  }, [approveOrder, order.id, navigate]);

  const openModal = useCallback(() => {
    Modal.confirm({
      title:
        'Você está prestes a aprovar esse pedido de venda, deseja continuar ?',
